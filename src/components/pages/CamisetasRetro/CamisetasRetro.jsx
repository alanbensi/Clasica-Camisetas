...
import './CamisetasRetro.css';

const CamisetasRetro = () => {
    const { fetchData, data, loading } = useFetchData(`/products/collections/Retros de coleccion`);
    ...
    return (
        <main>
            {loading ?
                ...
                :
                (<>
                    <h1 className='titRetroColeccion'>Camisetas retro de colección</h1>
                    <Container>
                        <Row>
                            {data.length !== 0 ?
                                (data.map((camiseta) => (
